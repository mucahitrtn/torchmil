...
def test_clam_sb_inst_eval(features):
    ...
    instance_loss, all_preds, all_targets = model.inst_eval(att, emb, model.inst_classifiers[0])
...
def test_clam_sb_inst_eval_out(features):
    ...
    att = torch.randn(features.shape[1])
    emb = torch.randn(features.shape[1], 10)
    instance_loss, p_preds, p_targets = model.inst_eval_out(att, emb, model.inst_classifiers[0])
    assert instance_loss.shape == ()
