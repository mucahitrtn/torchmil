...
def gtp_model():
    ...
    return GTP(in_shape=(10, 5), att_dim=512, n_clusters=8, n_layers=2, n_heads=4, use_mlp=True, dropout=0.1)
...
def test_forward_pass_with_loss(gtp_model, sample_input, sample_adj, sample_mask):
    Y_pred, loss_dict = gtp_model(sample_input, sample_adj, sample_mask, return_loss=True)
    assert Y_pred.shape == (2,), "Output shape should be (batch_size,)"
...
def test_compute_loss(gtp_model, sample_input, sample_adj, sample_mask, sample_labels):
    Y_pred, loss_dict = gtp_model.compute_loss(sample_labels, sample_input, sample_adj, sample_mask)
    assert Y_pred.shape == (2,), "Output shape should be (batch_size,)"
    assert isinstance(loss_dict, dict), "Loss should be a dictionary"
...
def test_predict(gtp_model, sample_input, sample_adj, sample_mask):
    Y_pred, inst_pred = gtp_model.predict(sample_input, sample_adj, sample_mask, return_inst_pred=True)
    assert Y_pred.shape == (2,), "Output shape should be (batch_size,)"
    assert inst_pred.shape == (2, 10), "Instance predictions shape should be (batch_size, bag_size)"
