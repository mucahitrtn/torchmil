...
from torchmil.nn import MultiheadSelfAttention, MultiheadCrossAttention, TransformerEncoder
...
class IIBMILDecoderLayer(torch.nn.Module):
    ...
    def forward(
        ...
        X = X + self.self_att(X, mask)
        X = self.norm1(X)

        U = U + self.cross_att(U, X, mask)
        U = self.norm2(U)
...
class IIBMILDecoder(torch.nn.Module):
    def __init__(
        ...
        self.layers = torch.nn.ModuleList([IIBMILDecoderLayer(dim, n_heads) for _ in range(n_layers)])
...
class IIBMIL(torch.nn.Module):
    def __init__(
        ...
        self.encoder = TransformerEncoder(att_dim, att_dim, n_heads, n_layers_encoder, use_mlp=True, add_self=False)
    ...
    def _inst_loss(
        ...
        loss_instance = self.criterion(y_pred, pseudo_labels)
    ...
    def _update_prototypes(
        ...
        X_enc_select_0 = X_enc_select[indice_0, :]
        X_enc_select_1 = X_enc_select[indice_1, :]

        for i in range(len(indice_0)):
            self.prototypes[0,:] = self.prototypes[0, :] * proto_m + (1 - proto_m) * X_enc_select_0[i]
        
        for i in range(len(indice_1)):
            self.prototypes[1,:] = self.prototypes[1, :] * proto_m + (1 - proto_m) * X_enc_select_1[i]
    ...
    def forward(
        ...
        U = self.query_embed.weight.unsqueeze(0).expand(batch_size, -1, -1) # (batch_size, n_queries, att_dim)
    ...
    def compute_loss(
        ...
        """
        Y_pred, y_pred, X_enc = self.forward(X, mask, return_inst_pred=True, return_X_enc=True)
        # inst_loss = self._inst_loss(X_enc, y_pred, mask)
        inst_loss = torch.zeros(1,)
