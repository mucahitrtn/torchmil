...
from torchmil.nn.utils import get_feat_dim, SinusoidalPositionalEncodingND, get_spatial_representation, get_sequential_representation
...
class SETMIL(MILModel):
    ...
    def __init__(
        ...
        if use_pmft:
            self.pmf_transf = PMFTransformerEncoder(
                ...
                kernel_list=pmft_kernel_list, stride_list=pmft_stride_list,
                padding_list=pmft_padding_list, dilation_list=pmft_dilation_list,
                n_heads=pmft_n_heads, use_mlp=pmft_use_mlp, dropout=pmft_dropout
            )

        self.se_transf = SETransformerEncoder(
            in_dim=att_dim, att_dim=att_dim, n_layers=set_n_layers, n_heads=set_n_heads, use_mlp=set_use_mlp, dropout=set_dropout,
    ...
    def forward(
        ...
        X = get_spatial_representation(X, coords)  # (batch_size, coord1, coord2, att_dim)
        ...
        if self.use_pmft:
            ...
            X = X.reshape(batch_size, self.att_dim, -1).transpose(1,2) # (batch_size, seq_len, att_dim)
            # seq_len = coord1 * coord2

        # Add class token
        cls_token = self.cls_token.expand(batch_size, -1, -1) # (batch_size, 1, att_dim)
        ...
        if return_att:
            if self.use_pmft:
                print("Warning: bag size has changed after PMF Transformer, cannot return attention values.")
                att = torch.zeros(batch_size, orig_bag_size, device=X.device)
            else:
                X, att = self.se_transf(X, return_att=True) # (batch_size, seq_len + 1, att_dim), (batch_size, seq_len+1, bag_size+1)
                att = att[:, 0, 1:]  # (batch_size, seq_len)
                att = att.view(batch_size, *coords_shape) # (batch_size, coord1, coord2)
