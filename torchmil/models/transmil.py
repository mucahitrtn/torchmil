...
class PPEG(torch.nn.Module):
    ...
    def forward(
        ...
        cnn_feat = feat_token.transpose(1, 2).view(batch_size, dim, H, W) # (batch_size, dim, H, W)
...
class TransMIL(MILModel):
    ...
    def forward(
        ...
        X = torch.cat([X, X[:,:add_length,:]], dim = 1) # (batch_size, padded_size*padded_size, att_dim)
